...
if(isset($_SERVER['HTTP_REFERER']) && !empty($_SERVER['HTTP_REFERER'])) {
	...
	if($http_referer != "https://libraryportal.000webhostapp.com/addedbyme.php"){
...
<body>
	...
	<div id="welcometext">
		<span>Added by <?php echo $_SESSION['name']; ?></span><br><br>LIST OF BOOKS
	...
	$query = $conn->prepare("SELECT ID, NAME, AUTHOR, ISSUED FROM BOOKS WHERE ADDEDBY = '$userid' ORDER BY ".$sort);
	$query->execute();
	...
	<script type="text/javascript">
		...
		$('#sort').on('change', function() {
			...
			xmlhttp.onreadystatechange = function() {
				if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					window.location.replace("addedbyme.php");
				}
			}
