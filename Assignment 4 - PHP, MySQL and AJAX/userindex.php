...
<body>
	...
	$query = $conn->prepare("SELECT ID, NAME, AUTHOR, ISSUEDTO FROM BOOKS ORDER BY ".$sort);
	$query->execute();
	...
	<script type="text/javascript">
		...
		function update(id, msg) {
			...
			else if(msg.indexOf("issueerror")) {
				document.getElementById('popup').innerHTML = "<span>Book removed by admin</span><img src=\"closepop.png\" onclick=\"popuprem()\">"
				$('#popup').slideDown(700);
				if(popupdisp()) {
					$("#welcometext").animate({top: "+=7vh"}, 700);
					$("#booktable").animate({top: "+=7vh"}, 700);
